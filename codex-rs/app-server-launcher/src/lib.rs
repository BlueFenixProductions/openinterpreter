...
fn record_startup_trace_event(event: &str) {
    ...
    let _ = writeln!(file, "{trace_event}");
}
...
pub async fn local_app_server_status(codex_home: &Path) -> Result<Option<LocalAppServerStatus>> {
    local_app_server_status_for_home(codex_home).await
}

pub async fn stop_local_app_server(codex_home: &Path) -> Result<StopLocalAppServerOutcome> {
...
fn resolve_default_app_server_binary() -> Result<PathBuf> {
    ...
    let sibling = current_exe
        ...
        .join(APP_SERVER_BINARY);
    if sibling.exists() {
        return Ok(sibling);
    }

    which::which(APP_SERVER_BINARY).with_context(|| {
        format!(
            "failed to locate `{APP_SERVER_BINARY}` next to `{}` or on PATH",
            current_exe.display()
        )
    })
}
